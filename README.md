...
│
├── config/
│   └── config.php            # Database credentials, timezone, and base URL setup
│
├── includes/
...
4. **Configure Database Settings (if necessary)**:
   Open `config/config.php` and verify credentials:
   ```php
...
1. **Upload Files**:
   Upload the project files to your cPanel `public_html/` or a subfolder (e.g., `public_html/website-monitor/`).
2. **Create MySQL Database**:
   * In cPanel, go to **MySQL® Databases**.
   * Create a new database (e.g., `username_monitor`) and a MySQL user.
   * Assign the user to the database with **ALL PRIVILEGES**.
3. **Import SQL Schema**:
   * Open **phpMyAdmin** in cPanel.
   * Select your new database and click **Import**.
   * Choose `database/database.sql` and execute.
...
2. Send `/newbot` and follow instructions to get your **Bot Token** (e.g., `123456789:ABCdefGhIJK...`).
3. Search for `@userinfobot` or add your bot to your alert group/channel and get your **Chat ID** (e.g., `987654321` or `-1001234567890`).
4. Log into the Admin panel and go to **Settings**.
5. Paste your **Bot Token** and **Chat ID**, toggle **Enable Instant Telegram Alerts**, and click **Save Telegram Settings**.
6. Click **🔔 Test Telegram Alert** to verify instant delivery.

---

## 🛡️ Security

* All database operations utilize PDO prepared statements with parameterized inputs.
* Passwords stored using standard `password_hash()` (bcrypt).
* Cross-Site Request Forgery (CSRF) tokens validated on all admin form submissions.
* Context-aware XSS escaping via `htmlspecialchars()`.
* Public pages are completely decoupled from admin logic and never reveal credentials or sensitive system information.
